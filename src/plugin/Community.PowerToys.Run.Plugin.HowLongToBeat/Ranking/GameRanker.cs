...
public static class GameRanker
{
    ...
    private static bool ContainsAllTokens(
        string candidate,
        IEnumerable<string> queryTokens)
    {
        var candidateTokens =
            candidate.Split(
                ' ',
        ...
        return queryTokens.All(
            candidateTokens.Contains);
    }

    private sealed record RankedGame(
        BridgeGame Game,
        double Score,
        int SourceIndex);
}