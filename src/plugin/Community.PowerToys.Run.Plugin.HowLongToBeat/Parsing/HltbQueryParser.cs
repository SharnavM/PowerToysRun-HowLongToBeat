...
public static partial class HltbQueryParser
{
    ...
    public static ParsedHltbQuery Parse(
        string? input)
    {
        var value =
            input?.Trim()
        ...
        ParseYear(string value)
    {
            ...
            || year is < 1900 or > 2999)
        {
            return (
                false,
                null,
                ParsedHltbQuery.Invalid(
                    "Invalid year",
                    "Use --year followed by a four-digit year."));
        }
    ...
    private static ParsedHltbQuery
        ConflictingDlcOptions()
    {
        return ParsedHltbQuery.Invalid(
            "Conflicting options",
            "Use either --dlc or --no-dlc.");
    }
    ...
    private static TokenizationResult Tokenize(
        string input)
    {
        var tokens =
            new List<string>();
