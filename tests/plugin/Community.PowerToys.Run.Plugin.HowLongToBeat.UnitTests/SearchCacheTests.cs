...
public sealed class SearchCacheTests
{
    ...
    public void NormalizedEquivalentQueryHitsCache()
    {
        ...
        cache.Set("Resident Evil 4", BridgeSearchMode.All, result);

        var found = cache.TryGet("resident   evil 4", BridgeSearchMode.All, out var cached);
    ...
    public void DifferentSearchModeDoesNotHit()
    {
        ...
        cache.Set("Elden Ring", BridgeSearchMode.All, Response());

        var found = cache.TryGet("Elden Ring", BridgeSearchMode.DlcOnly, out _);
    ...
    public void SuccessfulEntryExpires()
    {
        ...
        var cache = new SearchCache(successTtl: TimeSpan.FromMinutes(15), clock: () => now);

        cache.Set("Elden Ring", BridgeSearchMode.All, Response());

        now = now.AddMinutes(16);

        Assert.IsFalse(cache.TryGet("Elden Ring", BridgeSearchMode.All, out _));
    }
    ...
    public void EmptyEntryUsesShorterTtl()
    {
        ...
        var cache = new SearchCache(
            successTtl: TimeSpan.FromMinutes(15),
            emptyTtl: TimeSpan.FromMinutes(2),
            clock: () => now
        );

        cache.Set("nothing", BridgeSearchMode.All, new BridgeSearchResult([], 0));

        now = now.AddMinutes(3);

        Assert.IsFalse(cache.TryGet("nothing", BridgeSearchMode.All, out _));
    }
